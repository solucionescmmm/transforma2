        ...
        if (arrayData.data.length > 0) {
            ...
            for (let i = 0; i < array.length; i++) {
                ...
                let objInfoEmpresa = {
                    ...
                    arrPais: array[i]?.objInfoEmpresa[0]?.strLugarOperacion === "Local independiente" ?  JSON.parse(array[i]?.objInfoEmpresa[0]?.strPais || null) : JSON.parse(arrayEmpresario[i]?.strPais || null),
                    arrDepartamento: array[i]?.objInfoEmpresa[0]?.strLugarOperacion === "Local independiente" ? JSON.parse(array[i]?.objInfoEmpresa[0]?.strDepartamento || null) : JSON.parse(arrayEmpresario[j]?.strDepartamento || null),
                    arrCiudad: array[i]?.objInfoEmpresa[0]?.strLugarOperacion === "Local independiente" ? JSON.parse(array[i]?.objInfoEmpresa[0]?.strCiudad || null) : JSON.parse(arrayEmpresario[j]?.strCiudad || null),
